...
| Name | Type | Description |
| ---- | ---- | ----------- |
| deployer | address | The SquadSwap V3 deployer contract address |
...
```solidity
function getPool(address deployer, address tokenA, address tokenB, uint24 fee) public pure returns (contract ISquadV3Pool)
```
...
```solidity
function verifyCallback(address deployer, address tokenA, address tokenB, uint24 fee) public view returns (contract ISquadV3Pool pool)
```
...
```solidity
function verifyCallback(address deployer, struct SmartRouterHelper.PoolKey poolKey) public view returns (contract ISquadV3Pool pool)
```
