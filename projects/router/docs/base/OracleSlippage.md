...
```solidity
function getBlockStartingAndCurrentTick(contract IsquadV3Pool pool) internal view returns (int24 blockStartingTick, int24 currentTick)
```
...
```solidity
function getPoolAddress(address tokenA, address tokenB, uint24 fee) internal view virtual returns (contract IsquadV3Pool pool)
```
